...
class Handler extends Event {
  ...
  async run(msg) {
    ...
    if (blacklist.find(u => u.user === msg.author.id)) {
      this.bot.log.error(`Usuario incluido en la lista negra ${this.bot.tag(msg.author)} (${msg.author.id}) intentó ejecutar un comando en ${msg.channel.guild.name} (${msg.channel.guild.id}).`);
      return;
    }
    ...
    if (!msg.channel.memberHasPermission(this.bot.user.id, "embedLinks")) {
      return msg.channel.createMessage("Para funcionar correctamente, necesito permiso para **embed links**.");
    }
    ...
    if (cmd.clientperms) {
      ...
      if (!botperms.has(cmd.clientperms)) {
        return this.bot.embed("❌ Error", `Necesito el **${cmd.clientperms}** permiso para ejecutar ese comando.`, msg, "error");
      }
    }
    ...
    if (!msg.member.permission.has("administrator") && cmd.staff && guildconfig && guildconfig.staffRole && !msg.member.roles.includes(guildconfig.staffRole)) {
      return this.bot.embed("❌ Error", "Ese comando es solo para miembros del staff.", msg, "error");
    }
    ...
    if (cmd.args) {
      ...
      if (missingargs.length) {
        return this.bot.embed("❌ Error", `No se dio **${missingargs.map(a => a.name).join(" o ")}** .`, msg, "error");
      }
    }
