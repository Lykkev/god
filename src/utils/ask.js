  ...
  yesNo: async (bot, msg, retMsg = false) => {
    try {
      ...
      const [resp] = await waitFor("messageCreate", 15000, m => {
        ...
        // Checks for y/n
        if (m.content.toLowerCase() !== "s" && m.content.toLowerCase() !== "si" && m.content.toLowerCase() !== "n" && m.content.toLowerCase() !== "no") return false;
        return true;
      }, bot);

      // Checks for yes
      if (resp.content.toLowerCase() === "s" || resp.content.toLowerCase() === "si") {
        return retMsg ? {
          response: true,
          msg: resp,
        } : true;
