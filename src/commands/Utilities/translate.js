...
class translateCommand extends Command {
  constructor(...args) {
    super(...args, {
      ...
      description: "Traduce el texto entre los idiomas.",
      cooldown: 3,
    });
  }
  ...
  async run(msg, args) {
    ...
    if (!args.length) return this.bot.embed("❌ Error", "No se dio ningún texto para convertir.", msg, "error");
    ...
    if (!body || !body[0] || !body[0][0] || !body[0][0][1] || !body[2]) {
      return this.bot.embed("❌ Error", "No se encontró ninguna traducción.", msg, "error");
    }

    this.bot.embed(
      "🌍 Traducir",
      `Las traducciones pueden no ser 100% exactas. \n ${body[0][0][1]} **(${body[2].toUpperCase()})** ➜ ${body[0][0][0]} **(${locale.toUpperCase()})**`,
      msg,
    );
  }
}
