...
class reloadCommand extends Command {
  constructor(...args) {
    super(...args, {
      aliases: ["r"],
      args: "<command:string>",
      description: "Recarga uno o más comandos.",
      allowdisable: false,
      owner: true,
    });
  }
  ...
  run(msg, args) {
    ...
    args.forEach(arg => {
      ...
      if (!cmd) return fail.push({ id: arg.toLowerCase(), error: "Comando no encontrado." });
    ...
    return msg.channel.createMessage({
      embed: {
        title: `🔄 Recargados ${success.length === 1 ? success[0] : `${success.length} comandos`}.`,
