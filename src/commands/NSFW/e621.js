...
class e621Command extends Command {
  constructor(...args) {
    super(...args, {
      aliases: ["e6"],
      args: "[tags:string]",
      description: "Envía una imagen desde e621.",
      nsfw: true,
      cooldown: 3,
    });
  }
  ...
  async run(msg, args) {
    ...
    if (!body || body && body && !body.posts[0] || body && body.posts && !body.posts[0].file || body && body.posts && !body.posts[0].file.url || !body && body.posts && body.posts.length || body && body.success === false) {
      return this.bot.embed("❌ Error", "No se encontraron imagenes.", msg, "error");
    }
    ...
    if (body && body.posts && body.posts[random].file.url.endsWith(".webm") ||
      ...
      return this.bot.embed("❌ Error", `La publicación es un video. Puedes verlo [Aquí](${body.posts[0].file.url}).`, msg, "error");
    }
