...
class osuCommand extends Command {
  constructor(...args) {
    super(...args, {
      args: "<profile:string>",
      description: "Muestra información sobre un cuenta de osu! ",
      requiredkeys: ["osu"],
      allowdms: true,
      cooldown: 3,
    });
  ...
  async run(msg, args) {
    ...
    if (!body.length) return this.bot.embed("❌ Error", "Cuenta no encontrada.", msg, "error");
    const data = body[0];

    if (!data.pp_raw && !data.playcount && !data.level && !data.accuracy && !data.playcount) {
      return this.bot.embed("❌ Error", "La cuenta proporcionada no tiene datos.", msg, "error");
    }
    ...
    if (data.level > 0) fields.push({ name: "Nivel", value: formatNumber(data.level), inline: true });
    if (data.accuracy > 0) fields.push({ name: "Precisión", value: `${Math.round(data.accuracy)}%`, inline: true });
    if (data.playcount > 0) fields.push({ name: "Jugó", value: formatNumber(data.playcount), inline: true });
    if (data.total_score > 0) fields.push({ name: "Puntaje total", value: data.total_score, inline: true });
    if (data.join_date) fields.push({ name: "Fecha de Ingreso", value: format.date(data.join_date), inline: true });
