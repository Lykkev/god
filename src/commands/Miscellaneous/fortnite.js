...
class fortniteCommand extends Command {
  constructor(...args) {
    super(...args, {
      ...
      description: "Devuelve las estadísticas de la cuenta de Fortnite.",
  ...
  async run(msg, args) {
    ...
    if (!["psn", "xbox", "pc"].includes(platform) && platform !== undefined) {
      return this.bot.embed("🎮 Fortnite", "Las plataformas válidas son `pc`, `psn`, and `xbox`.", msg);
    }

    // Sends the first embed
    const fortnitemsg = await this.bot.embed("🎮 Fortnite", "Obteniendo información...", msg);
    ...
    if (!body || body.error || !body.lifeTimeStats) return this.bot.embed.edit("❌ Error", "No se encontró información.", fortnitemsg, "error");
    ...
    fortnitemsg.edit({
      embed: {
        title: `🤙 Estadísticas de Fortnite para **${body.epicUserHandle}** en **${body.platformNameLong}**`,
        color: 0x6FC8F0,
        fields: fields,
        footer: {
