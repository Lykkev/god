...
class remindCommand extends Command {
  constructor(...args) {
    super(...args, {
      aliases: ["remindme", "recordatorio", "rm"],
      args: "[time:string] [reminder:string]",
      description: "Te envía un recordatorio más tarde.",
      cooldown: 3,
    });
  ...
  async run(msg, args) {
    ...
    if (!args[0] || args[0].toLowerCase() === "list") {
      ...
      if (!db.length) return this.bot.embed("⏰ Recordatorio", "No tienes recordatorios. Establecer uno usando remindme <tiempo> <recordatorio>.", msg);
    ...
    if (args[0] && (args[0].toLowerCase() === "remove" || args[0].toLowerCase() === "delete")) {
      if (!args[1] || !args[1].length) return this.bot.embed("❌ Error", "Proporcionaste una identificación no válida.", msg, "error");

      // If a user didn't set the reminder they provided
      const db = await this.bot.db.table("reminders").get(args[1]).run();
      if (db.user !== msg.author.id) return this.bot.embed("❌ Error", "No creaste ese recordatorio.", msg, "error");
      ...
      if (reminder.skipped || reminder.errors) return this.bot.embed("❌ Error", "Recordatorio no encontrado.", msg, "error");

      // Clears the timeout
      const handle = this.timeoutHandles.find(h => h.id === args[1]);
      if (handle) clearTimeout(handle);
      return this.bot.embed("⏰ Recordatorio ", "Recordatorio eliminado.", msg);
    }
    ...
    if (val < 1000) return this.bot.embed("❌ Error", "Proporcionaste una cantidad de tiempo no válida.", msg, "error");
    const finaldate = new Date().getTime() + val;

    if (finaldate > new Date().getTime() + 2142720000) {
      return this.bot.embed("❌ Error", "La cantidad de tiempo debe ser inferior a 25 días.", msg, "error");
    }
    ...
    if (!reminderDatabase.errors) {
      const handle = setTimeout(async r => {
        ...
        await dm.createMessage({
          embed: {
            ...
            color: this.bot.embed.color("general"),
            footer: {
              text: `Establecer en ${format.date(r.set, false)}`,
      ...
      msg.channel.createMessage({
        embed: {
          title: "⏰ Recordatorio",
          description: `Te recordaré ${args.join(" ")} en ${timeArg.split(" ").filter((a, i) => !(a.length === 0 || (a === " " && i === args.length))).join(" ")}.`,
          color: this.bot.embed.color("general"),
          fields: [{
            name: "ID",
            value: reminder.id,
