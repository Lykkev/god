...
class urbanCommand extends Command {
  constructor(...args) {
    super(...args, {
      ...
      args: "<word:string>",
      description: "Devuelve una definición del Urban Dictionary.",
      allowdms: true,
      cooldown: 3,
    });
  }
  ...
  async run(msg, args) {
    ...
    if (!body) return this.bot.embed("❌ Error", "No se pudo enviar la definición. Vuelve a intentarlo más tarde.", msg, "error");

    // Sorts by highest rated definition
    const topword = body.list.sort((a, b) => b.thumbs_up - a.thumbs_up);
    const word = topword[0];
    if (!topword || !word || !word.definition) return this.bot.embed("❌ Error", "No se encontró una definición.", msg, "error");
    ...
    const fields = [];
    if (word.example) fields.push({ name: "Ejemplo", value: `${word.example}` });
