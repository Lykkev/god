...
class setupCommand extends Command {
  constructor(...args) {
    super(...args, {
      ...
      description: "Configura el bot o ve la configuración.",
      allowdisable: false,
      cooldown: 3,
    });
  }
  ...
  async run(msg, args) {
    ...
    const omsg = await msg.channel.createMessage({
      embed: {
        title: "🔧 Setup",
        description: this.bot.config.homepage ? `Si necesitas ayuda contacta con Lykkev#0707` : "",
        color: this.bot.embed.color("general"),
    ...
    async function getCategory(m, bot, editMsg) {
      if (editMsg) m.edit({
        embed: {
          ...
          footer: {
            text: `Pedido por ${bot.tag(msg.author)}`,
            icon_url: msg.author.dynamicAvatarURL(),
          },
        },
      });
    ...
    if (category.error === "timeout") return this.bot.embed.edit("❌ Error", "Tiempo de espera alcanzado, saliendo de la configuración.", omsg, "error");
    ...
    await waitFor("messageReactionAdd", 60000, async (message, emoji, uid) => {
      ...
      if (setting.type === "bool") {
        ...
        await waitFor("messageReactionAdd", 60000, async (m, emojii, user) => {
          ...
          this.bot.embed.edit(
            `🔨 Castigos por ${setting.pickerLabel}`,
        ...
        // Asks for a response
        this.bot.embed.edit("🔧 Setup", `Responde con lo pedido **${setting.type || setting.type}**. Tienes **60 segundos** para responder.`, omsg);
        await waitFor("messageCreate", 60000, async m => {
          ...
          if (setting.type !== "bool" && !result || typeof result === "string" && result.startsWith("No")) {
            const errormsg = await this.bot.embed(
              "❌ Error",
              `Invalid ${setting.type}${Math.abs(cooldown - 2) === 0 ? "" : `; **${Math.abs(cooldown - 2)}** los intentos se fueron antes de salir.`}`,
              msg,
              "error",
            );
          ...
          if (setting.type === "roleArray" && setting.maximum && result.length > setting.maximum) {
            return this.bot.embed("❌ Error", `No puedes configurar más de ${setting.maximum} roles.`, msg, "error");
          }

          if (setting.type === "channelArray" && setting.maximum && result.length > setting.maximum) {
            return this.bot.embed("❌ Error", `No puedes configurar más de ${setting.maximum} canales.`, msg, "error");
          }
          ...
          if (setting.type === "number" && setting.maximum && setting.maximum && (setting.minimum > result || setting.maximum < result)) {
            return this.bot.embed(
              "❌ Error",
              `El número debe estar por debajo ${setting.maximum} y más ${setting.minimum}.`,
              msg,
              "error",
            );
          }
          ...
          m.delete();
          const setmsg = await this.bot.embed("✅ Listo", `**${setting.id}** se ha establecido en **${result}**.`, msg, "success");
