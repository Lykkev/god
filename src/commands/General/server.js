...
class serverCommand extends Command {
  constructor(...args) {
    super(...args, {
      aliases: ["servidor", "guildinfo", "serverinfo"],
  ...
  run(msg, args) {
    ...
    fields.push({
      name: "Miembros",
      value: `${members} ${members === 1 ? "miembro" : "miembros"}${bots > 0 ? `, ${bots} bot${bots === 1 ? "" : "s"}` : ""}`,
      inline: true,
    });

    fields.push({
      name: "Canales",
      value: `${text} texto${voice > 0 ? `, ${voice} voz` : ""}`,
      inline: true,
    });
    ...
    if (guild.explicitContentFilter > 0) fields.push({
      ...
      value: `Nivel ${guild.explicitContentFilter}`,
      inline: true,
    });

    if (guild.verificationLevel > 0) fields.push({
      name: "Verificación",
      value: `Nivel ${guild.verificationLevel}`,
      inline: true,
    });
    ...
    if (guild.features.length) fields.push({
      name: "Caracteristicas del Servidor",
